...
namespace gaia {
	...
	struct Vector3
	{
		...
		Vector3 operator + (const Vector3& y) const { return Vector3(x + other.x, y + other.y, z + other.z); }
		Vector3 operator - (const Vector3& y) const { return Vector3(x - other.x, y - other.y, z - other.z); }
		Vector3 operator * (const Vector3& y) const { return Vector3(x * scalar, y * scalar, z * scalar); }
